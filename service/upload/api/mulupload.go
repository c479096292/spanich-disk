...
import (
	...
	"strings"
	"time"
	"github.com/c479096292/spinach-disk/cache/redis"
	"os"
...
func InitialMultipartUploadHandler(c *gin.Context) {
	...
	// 4. 将初始化信息写入到redis缓存
	redis_cli.Do("HSET", "MP_"+upInfo.UploadID, "chunkcount", upInfo.ChunkCount)
	redis_cli.Do("HSET", "MP_"+upInfo.UploadID, "filehash", upInfo.FileHash)
	redis_cli.Do("HSET", "MP_"+upInfo.UploadID, "filesize", upInfo.FileSize)
...
func UploadPartHandler(c *gin.Context) {
	...
	// 4. 更新redis缓存状态
	redis_cli.Do("HSET", "MP_"+uploadID, "chkidx_"+chunkIndex, 1)
...
func CompleteUploadHandler(c *gin.Context) {
	...
	res, err := redis_cli.Do("HGETALL", "MP_"+upid).Result()
	data, ok := res.([]interface{})
	if !ok {
		log.Println("HGETALL mp_block exception")
	}
	if err != nil {
		c.JSON(
			http.StatusOK,
