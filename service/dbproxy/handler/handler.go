...
import (
	...
	"context"
	"encoding/json"
	"github.com/c479096292/spinach-disk/service/dbproxy/mapper"
...
func (db *DBProxy) ExecuteAction(ctx context.Context, req *dbProxy.ReqExec, res *dbProxy.RespExec) error {
	resList := make([]model.ExecResult, len(req.Action))
	...
	for idx, singleAction := range req.Action {
		...
		resList[idx] = execRes[0].Interface().(model.ExecResult)
	}
